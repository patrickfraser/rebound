...
void integrator_part1(){
	// DRIFT
	_N_active = ((N_active==-1)?N:N_active)-N_megno;
	integrator_wh_to_jacobi();
	drift_wh(dt/2.);
	for (int i=N-N_megno;i<N;i++){
		particles[i].x += dt/2.*particles[i].vx;
		particles[i].y += dt/2.*particles[i].vy;
		particles[i].z += dt/2.*particles[i].vz;
	}
...
#pragma omp parallel for schedule(guided)
	for(int i=_N_active;i<N-N_megno;i++){
